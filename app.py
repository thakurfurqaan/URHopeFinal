        ...
        try:
            ...
            if account:
                ...
                if password == confirmpassword:
                    c.execute(
                        'insert into members (name, username, phone, pin, role, services, password ) values (%s, %s, %s, %s, %s, %s, md5(%s))'
                        , (
                            name,
                            username,
                            phone,
...
def login():
    if request.method == 'POST' and 'username' in request.form \
        ...
        try:
            ...
            c = db.cursor()
            c.execute(
                'select id,name, username, password, role, phone, pin, regno, age, sex, currProfile, address, social, services, branch, about, govtID,website from members where username = %s and password = md5(%s)'
                , (username, password))
            account = c.fetchone()
    ...
    if not session.get('logged_in'):
        ...
        if role == 'V' or role == 'v':
            ...
            if request.method == 'POST' and 'name' in request.form \
                ...
                exe.execute(
                    'UPDATE members SET name=%s,website=%s, social=%s,services=%s,address=%s,regno=%s,branch=%s,phone=%s,pin=%s,about=%s WHERE username = %s '
                    , (
                        name,
                        website,
                        social,
...
def create_task():
    if not session.get('logged_in'):
        ...
        if request.method == 'POST':
            ...
            exe = connect.cursor()
            exe.execute(
                'insert into task (task, grp, website, location, phone, vol_num, task_det, t_type, abt_grp, grp_email) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)'
                , (
                    task,
                    grp,
                    website,
...
def edit_task(id):
    ...
    if not session.get('logged_in'):
        ...
        if request.method == 'POST':
            ...
            exe.execute(
                'UPDATE task SET task=%s, grp=%s, website=%s, location=%s, phone=%s, vol_num=%s, task_det=%s, t_type=%s where id= %s'
                , (
                    task,
                    grp,
                    website,
...
def delete_task(id):
    ...
    if not session.get('logged_in'):
        ...
        x = "Task with ID " + str(
            id) + " has been deleted successfully. If you had any volunteer applied for this task their application has also been deleted."
        flash(x)
...
def task_list():
    if not session.get('logged_in'):
        ...
        return render_template('task_list_v.html', id=id, l=len(app_data), applied=applied, len=len(data), data=data,
                               app_data=app_data)
    else:
        return redirect(url_for('login'))
...
def apply_task(id):
    ...
    if data[0][11] < data[0][7]:
        ...
        c.execute(
            'INSERT INTO application(grp_email,vol_email,task_id,vol_id,grp_name,vol_name,task_name,vol_phone) values(%s,%s,%s,%s,%s,%s,%s,%s)',
            (data[0][12], session['username'], data[0][0], session['user_id'], data[0][3], session['name'], data[0][1],
             session['phone']))
        db.commit()
        c.close()
        db.close()
        ...
        body = "Dear " + data[0][3] + ",\n\n" + session['name'] + " has applied for the task " + data[0][
            1] + " which has an ID: " + str(
            data[0][0]) + ".\n\nSo far the total number of applicaion for this task is " + str(val) + " and " + str(
            data[0][
                7] - val) + " more volunteers are required.\n\nClick on the link for more details,\nhttp://www.urhope.in\n\n\nRegards,\nURHope Team"
        msg = f"Subject: {subject}\n\n{body} "

        server.sendmail(
            'ur_hope_email_address',  # email ID of URHope or use your email ID for testing
            str(data[0][12]),
            # email id of receiver ie. data[0][12], NGO's email ID or use any other known email ID for testing
            msg
        )

        if (val == data[0][7]):
            body = "Dear " + data[0][3] + ",\n\nFor the task " + data[0][
                1] + ", you have sufficient volunteers. Now you can proceed for further steps.\n\n\nRegards,\nURHope Team"
            msg = f"Subject: {subject}\n\n{body} "
            server.sendmail(
...
def how_is_the_task(id):
    ...
    x = "Hey " + session['name'] + ",for the task '" + data[0][1] + "', total number of applications are " + str(
        data[0][11]) + ". You need " + str(data[0][7] - data[0][11]) + " more volunteer to start the task."
    flash(x)
...
def search_pincode(pincode):
    ...
    query = "select m.pin, phone, services, statename from members m join podata p on m.pin = p.pin where m.role='n' AND (" + where[
                                                                                                                              :-4] + ")"
    c.execute(query)
    data = c.fetchall()
    if data:
...
def serch_result():
    if request.method == "POST":
        ...
        if lpin == 6:
            ...
            db = get_db()
            c = db.cursor()
            c.execute('SELECT * FROM members WHERE services=%s and pin=%s and role=%s ORDER BY pin ASC',
                      (name, pin, role))
            data = c.fetchall()
            l = len(data)
            db.commit()
...
def del_vol(id):
    ...
    if len(gdata) > 0:
        for i in gdata:
            c.execute('SELECT vol_applied FROM task WHERE grp_email=%s',
                      (i[0]))  # deletes all the volunteer applications
            val = c.fetchone()
            val = val[0] - 1
